...
This is a basic Widget that forms a part of a Group (see [Grouping](../../product-guide/content-and-app-layout/editing-and-laying-out--reference/widget-grouping) ). A group of Radio Buttons is used to allow the Respondent to make a single selection from several options.

When a Radio Button is part of a group, the group setting can be set to check for mandatory completion, and its Uniqueness can be set (For use with [Relational Apps)](../../product-guide/advanced-features/accessing-data-in-other-apps,-databases-and-apis/relational-apps)
